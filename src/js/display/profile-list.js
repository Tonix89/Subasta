  ...
  getList(listUrl).then((data) => {
    ...
    if (data) {
      ...
      data.forEach((listing) => {
        ...
        // console.log(id, title, description, media, tags, endsAt, bids);
        ...
        if (bids.length === 0) {
          ...
          <div class="d-flex overflow-hidden">
            ...
            <h6 class="card-text text-overflow mb-0 p-1">${bids.name}</h6>
          </div>
          <div class="d-flex ps-2">
            <img src="assets/icon/coins.svg" alt="coins icon" class="coins-icon" />
            <h6 class="mb-0 ps-1">${bids.amount}</h6>
          </div>
        </div>`;
        }
        ...
        <div class="col">
          <div class="card h-100 border border-3 border-primary p-1">
            ...
            <div class="card-footer border-top-0 text-center">
              <button type="button" class="btn btn-primary text-success w-75 fw-bold listing-viewMore-btn" >View More</button>
            </div>
          </div>
        </div>`;
      });
